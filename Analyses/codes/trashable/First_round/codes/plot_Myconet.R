...
net=read.table("verbose/Iron_deprivation_effect_at_Stat_fv.txt")

## The backbone, previous to any manipulation on our end has 2907 nodes (proteins) and 8042 interactions

nrow(net)
# 7128
## Number of significant links:
length(which(net$log2FC_gene_A>0 & net$log2FC_gene_B>0))
## 1745
length(which(net$Fdr<0.05 & net$log2FC_gene_A>0 & net$log2FC_gene_B>0))
# 743
743/1745
# 42.6% of links are significant


### P value histogram of links, an dvolcano plot
n_genes=nrow(net)

library(ggplot2)
hist <- ggplot(net)+
geom_histogram(aes(x=p),color="black",fill="lightblue",bins = (n_genes/(n_genes/50)),boundary=0)+
  scale_x_continuous(limits = c(0, 1)) +
  theme_minimal()+
labs(x="PValues", y = "Number of links (global PPIN)")+
  theme(plot.title = element_text(hjust = 0.5))

### From the original PPIN, we found 1857 significantly over-represented interactions ( 23.1% of the total number of links reported in Wang et al., see Fig. left: p value histogram, right: volcano plot)

### Among these, 1745 connect pairs of proteins both of which have possitive effect sizes upon iron deprivation at the stationary phase in our experiment (42.6% are individually significantly up-regulated).

volcano <- ggplot(net)+
geom_point(aes(x=weight, y=-log10(Fdr)),color="dodgerblue",alpha=0.2)+
theme_minimal()+ylim(0,40)+
labs(x="logFC", y = "-log10FDR")

analyzed=net[(which(net$log2FC_gene_A>0 & net$log2FC_gene_B>0)),c(1,2,17)]

genstats=net[(which(net$log2FC_gene_A>0 & net$log2FC_gene_B>0)),]
one=genstats[,c(1,15)]
two=genstats[,c(2,16)]
colnames(two)=colnames(one)
all=rbind(one,two)
all=all[which(!duplicated(all[,1])),]

library(igraph)
gr=graph_from_data_frame(analyzed, directed =FALSE,vertices=NULL)
dd=degree_distribution(gr,cumulative=TRUE)
degree_distr=data.frame(overlap=c(0:(length(dd)-1)),cumulative_p=dd)


distro <- ggplot(degree_distr)+
geom_line(aes(x=log(overlap), y=log(cumulative_p)),color="black")+
geom_point(aes(x=log(overlap), y=log(cumulative_p)),color="dodgerblue")+
theme_minimal()+xlim(0,4)+
labs(x="Overlap o*", y = "P(o>o*)")

log_degree_distr=log10(degree_distr)[2:nrow(degree_distr),]

scale_free_fit=lm(cumulative_p~overlap, data=log_degree_distr)
summary(scale_free_fit)
# gamma=2.32+-0.07, p=1E-33 --> It is a SF net.

pdf("a.pdf",height=4,width=5)
print(hist)
dev.off()


pdf("b.pdf",height=4,width=5)
print(volcano)
dev.off()


pdf("c.pdf",height=4,width=5)
print(distro)
dev.off()
...
analyzed_base=analyzed
analyzed_base$weight=1

library(igraph)
gr_base=graph_from_data_frame(analyzed_base, directed =FALSE,vertices=NULL)
dd_base=degree_distribution(gr_base,cumulative=TRUE)
degree_distr_base=data.frame(overlap=c(0:(length(dd_base)-1)),cumulative_p=dd_base)


distro_base <- ggplot(degree_distr_base)+
geom_point(aes(x=log(overlap), y=log(cumulative_p)),color="dodgerblue")+
theme_minimal()+
labs(x="Overlap o*", y = "P(o>o*)")
...
# 650
## Luwgo habría
a=length(which(net$Fdr<0.05))
b=length(which(net$Fdr<0.05 & net$log2FC_gene_A>0 & net$log2FC_gene_B>0))
c=length(which(net$Fdr<0.05 & net$log2FC_gene_A<0 & net$log2FC_gene_B<0))
a-b-c
## 464 links entre genes que van cada uno a la suya.

## Fraction:
length(which(net$Fdr<0.05))/nrow(net)
## 0.2605219
length(which(net$Fdr<0.05))/8042
## 0.2309127

743/8042
## 0.0923

length(unique(c(net[,1],net[,2]))))
