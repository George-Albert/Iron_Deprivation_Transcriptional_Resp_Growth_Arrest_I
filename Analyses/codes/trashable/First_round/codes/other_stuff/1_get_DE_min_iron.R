...
### Where colSums sum in the columns
### t is the transpose

options(width=10000)#####examines a variety of global options which 
                    #####affect the way in which R computes and displays
                    #####its results
## what does n means?
{
    ul=function(tab,n=5)
...
culture_to_select="C2"

metadata<-metadata[which(metadata$Culture==culture_to_select),]
    reads = reads[,which(colnames(reads) %in% rownames(metadata))]
    fpkms = fpkms[,which(colnames(reads) %in% rownames(metadata))]
    
    ### Create a column with culture+Fe+Iron_column_content
    metadata$short_setup=paste0(metadata$Culture,"_Fe_",metadata$Iron)
    
    ### cut the fpkms table, keep the Yes samples
    medias_within_group=fpkms[,1:2]
    
    ### Select the columns with no data repeted from the short_setup column
    ...
    {
        chunk=fpkms[,which(metadata$short_setup %in% colnames
                           (medias_within_group)[i])]
        chunk_reads=reads[,which(metadata$short_setup %in% colnames
                                 (medias_within_group)[i])]
    ...
    #### in the for loop we apply the mean and the min to the rows
    ...
    ## Here, with a criterium of being above 10 fpkms in at least one setup,
    ##I only trash 7% of genes, and the genes kept are above a mean 
    ## of reads=9 in at least one of the setups.???
    ...
    metadata$short_setup=relevel(metadata$short_setup,ref=paste0(culture_to_select,"_Fe_YES"))
    ### Check NA values
    #indx<-is.na(reads)
    #reads[indx]<-0
    ...
    pdf(file="ojo_con_esto_voom_plot_no_C2.pdf")
    par(mar = c(6, 6,6,6))
    ### Transform data to log2 counts per million (logCPM),
    ### mean variance relationship to compute weights
    ...
    ### First, I build effect sizes, normalized t-statistics, pvalues and 
    ###fdr matrices from contrasts 1, 3 and 5 (accessible without contrast.fit.
    ...
    for(i in 1:ncol(fdrs))
    {
        ...
        #print(paste(colnames(fdrs)[i],length(which(fdrs[,i]<0.1 & 
        #abs(betas[,i])>0)),length(which(fdrs[,i]<0.05 & abs(betas[,i])>0))))
    }
    ...
    ps=ps[,c(3,6,4)]
    fdrs=fdrs[,c(3,6,4)]
    ash_fdrs=ash_fdrs[,c(3,6,4)]
    ash_betas=ash_betas[,c(3,6,4)]

    errors=errors[,c(3,6,4)]

    colnames(betas)=c("c1","c3","c5")
    colnames(ts)=c("c1","c3","c5")
    colnames(ps)=c("c1","c3","c5")
    colnames(fdrs)=c("c1","c3","c5")
    colnames(ash_fdrs)=c("c1","c3","c5")
    colnames(ash_betas)=c("c1","c3","c5")

    colnames(errors)=c("c1","c3","c5")

    
    DE_results=list(betas=betas,ash_betas=ash_betas,ts=ts,ps=ps,fdrs=fdrs,
                    ash_fdrs=ash_fdrs,errors=errors,dofs=dofs)
...
{
    system(paste0("mkdir -p outputs_JAC/DE_portrait/",name,"/target_sets/BH/",names(targets)[i]))
    system(paste0("mkdir -p outputs_JAC/DE_portrait/",name,"/target_sets/ash/",names(targets)[i]))

    writer_set(targets[[i]]$up,path=paste0("outputs_JAC/DE_portrait/",name,"/target_sets/BH/",names(targets)[i]),nombre_file="/up")
    writer_set(targets[[i]]$down,path=paste0("outputs_JAC/DE_portrait/",name,"/target_sets/BH/",names(targets)[i]),nombre_file="/down")
    writer_set(targets[[i]]$all,path=paste0("outputs_JAC/DE_portrait/",name,"/target_sets/BH/",names(targets)[i]),nombre_file="/all")
    
    writer_set(ash_targets[[i]]$up,path=paste0("outputs_JAC/DE_portrait/",name,"/target_sets/ash/",names(targets)[i]),nombre_file="/up")
    writer_set(ash_targets[[i]]$down,path=paste0("outputs_JAC/DE_portrait/",name,"/target_sets/ash/",names(targets)[i]),nombre_file="/down")
    writer_set(ash_targets[[i]]$all,path=paste0("outputs_JAC/DE_portrait/",name,"/target_sets/ash/",names(targets)[i]),nombre_file="/all")

    get_signal_figures(x=betas,y=-log10(fdrs),xalt=ash_betas,yalt=-log10(ash_fdrs),z=ps,i,th_x=th_beta,th_y=-log10(th_fdr),path=paste0("outputs_JAC/DE_portrait/",name,"/signal_plots/"),nombre_file=names(targets)[i])
    
}

get_signal_barplot(hits=hits,path=paste0("outputs_JAC/DE_portrait/",name,"/barplots/"),nombre_file="BH")
get_signal_barplot(hits=ash_hits,path=paste0("outputs_JAC/DE_portrait/",name,"/barplots/"),nombre_file="ash")
...
datos$Culture=factor(datos$Culture,levels=c("C5","C1","C6"))
datos$short_setup=factor(datos$short_setup,levels=c("C5_Fe_YES","C5_Fe_NO","C1_Fe_YES","C1_Fe_NO","C6_Fe_YES","C6_Fe_NO"))
...
system("mkdir -p outputs_JAC/summary_figures")
pdf("outputs_JAC/summary_figures/pca.pdf",width=6,height=4.5)
print(pl)
dev.off()
...
system("mkdir -p outputs_JAC/summary_figures")
pdf("outputs_JAC/summary_figures/pca_1_3.pdf",width=6,height=4.5)
print(pl)
dev.off()


pl=ggplot(datos)+geom_point(aes(x=PC2,y=PC3,color=Culture,fill=short_setup)
                            ,shape=21,size=3,stroke=1.5)+scale_colour_manual(values=colores_base)+scale_fill_manual(values=fill_base)+xlab("PC1: 50.3% variance ~growth phase")+ylab("PC1: 14.8% variance ~Dextrose, Iron (at stat phase only)")
...
pdf("outputs_JAC/summary_figures/beta_densities.pdf",width=10,height=6)
print(pl)
dev.off()
...
pl=ggplot(betas_melt)+geom_density(aes(x=value,fill=Var2,color=Var2),alpha=0)+ylim(0,4)
pdf("outputs_JAC/summary_figures/ash_beta_densities.pdf",width=10,height=6)
print(pl)
dev.off()
...
### Beta distributions.
###
##### of C6_Fe_YES_1, C6_Fe_YES_2, C5_Fe_NO_1, C5_Fe_NO_2
...
### C5_Fe_NO_1, C5_Fe_NO_2, C6_Fe_NO_1, C6_Fe_NO_2
chunk=reads[,c(1:4)]
meta_chunk=metadata[which(rownames(metadata)%in%colnames(chunk)),]
...
names(DE_results)
    ...
    ####  C6_Fe_YES_1, C6_Fe_YES_2, C5_Fe_NO_1, C5_Fe_NO_2
