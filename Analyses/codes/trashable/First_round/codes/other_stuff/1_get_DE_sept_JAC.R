...
## what does n means?
{
    ...
    metadata<-metadata[!(metadata$Culture=="C2"),]
    reads = reads[,!grepl("^C2",names(reads))]
    fpkms = fpkms[,!grepl("^C2",names(fpkms))]
...
##     reads = reads[,which(colnames(reads) %in% rownames(metadata))]
    ...
    number_cul = 6
    medias_within_group=fpkms[,c((number_cul + 1):12)]
...
dir.create("outputs")
    ...
    pdf(file="outputs/voom_plot_no_C2.pdf")
    par(mar = c(6, 6,6,6))
    ...
    DE_results=list(betas=betas,ts=ts,ps=ps,fdrs=fdrs,errors=errors,dofs=dofs)
    ...
    DE_output=list(betas=betas_ext,ts=ts_ext,ps=ps_ext,fdrs=fdrs_ext,errors=errors_ext,dofs=dofs_ext)
...
for(i in 1:length(targets))
{
    ...
    get_signal_figures(x=betas,y=-log10(fdrs),xalt=betas,yalt=-log10(fdrs),z=ps,i,th_x=th_beta,th_y=-log10(th_fdr),path=paste0("outputs_JAC/DE_portrait/",name,"/signal_plots/"),nombre_file=names(targets)[i])
...
output=list(targets_BH=targets,hits=hits)
