...
name_gene <- function(path,tp_name,pr_data,Condition){
  ...
  partition_path <- paste0(path,"/partition_IDES.dat")
...
tp_gene <- function(path,tp_name,pr_data,Condition){
  ...
  tp_path <- paste0(path,"/",tp_name)
  partition_path <- paste0(path,"/partitionsIDES.dat")
...
verbose_path <- "~/Documents/PhD_Zaragoza/Collaboration_UAM_unizar/Outputs_def"
setwd(verbose_path)

path3 <- "OSLOM_final"

mylist3 <- name_gene(path=path3,tp_name="tp.dat",pr_data=IDEE_osl,Condition="IDES")

mylist3_tp <- tp_gene(path=path3,tp_name="tp.dat",pr_data=IDEE_osl,Condition="IDES")
